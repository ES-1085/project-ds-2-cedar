...
all_physicians %>% 
  ggplot() + 
  geom_point(aes(x = all_md_count, y = all_gp_md_count), position = "jitter", alpha = 0.5) +
  geom_smooth(aes(x = all_md_count, y = all_gp_md_count), method = "lm", se = FALSE)
...
all_physicians %>% 
  mutate(test = all_gp_md_count / all_md_count)
```

```{r physician-change-model}
hpsa_pc_provider_counts %>% 
  ggplot(aes(x = pc_physician_count, y = md_pc_count)) + 
...
pc_provider_change_aug %>% 
  ...
  geom_smooth(method = "lm", se = FALSE, color = "#009796") +
  theme_minimal() +
  labs(title = "Fitted and Residual Values", subtitle = "Primary care (MD) divided by all primary care physicians.") 
...
(dental_provider_change <- linear_reg() %>% 
  set_engine("lm") %>% 
  fit(active_dentist_count/popn_est_count ~ dent_data_year + pct_below_pov_line, data = dental_care))
